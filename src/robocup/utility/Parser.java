...
import robocup.memory.SenseMemory;
 ...
 * @class Parser
 *
 *        This class takes in the the messages sent by the parser and parses
 *        them into information that can be stored in Memory and used by
 *        Players.
 *
 *
 */
public class Parser {
	 ...
	 *
	 * @param inputPacket
	 *            The init message from the server
	 * @param mem
	 *            the player's memory
	 ...
	 *       of the field the player starts on, the Player's uniform number and
	 *       the play mode, which is "before_kickoff."
	 */
	public void initParse(String inputPacket, Memory mem) {
		...
		if (splitPacket[0].compareTo("init") == 0) {
			mem.setSide(splitPacket[1]);
			mem.setuNum(Integer.valueOf(splitPacket[2]));
...
//				.println("(player #" + mem.getuNum() + " side: " + mem.getSide() + " Play mode: " + mem.getPlayMode());
	 ...
	 *
	 * @param inputPacket
	 *            the incoming String message from the server
	 * @param InfoMem
	 *            the Memory to store all the information in
	 ...
	 *       (sense_body) message, or ObjInfos from the (see) message, or the
	 *       playMode from the referee (hear) message
	 */
	public void Parse(String inputPacket, Memory InfoMem) {
		...
		if ((packetType[0].compareTo("see") == 0) || (packetType[0].compareTo("sense_body") == 0)
				|| (packetType[0].compareTo("hear") == 0)) {
			// the time from the message
			int time = Integer.parseInt(packetType[1]);

			// Call parse method based on packet type in position [0] (either
			// see, sense_body, or hear)
	 ...
	 *
	 * @param InfoMem
	 *            the Memory to store the play mode in
	 * @param splitPacket
	 *            the split up message from the server
	 */
...
//		System.out.println(Arrays.toString(splitInfo));
		if (splitInfo[2].compareTo("referee") == 0) {
			InfoMem.setPlayMode(splitInfo[3]);
		} else if (splitInfo[2].equals("self")) {
			// TODO da fare
		} else if (splitInfo[3].equals("our")) {
			Message msg = new Message(Integer.parseInt(splitInfo[1]), Integer.parseInt(splitInfo[2]), splitInfo[3],
					Integer.parseInt(splitInfo[4]), splitInfo[5]);
	 ...
	 *
	 * @param seeArray
	 *            The ArrayList that stores all the ObjInfos from the message.
	 *            This will be saved in the Memory
	 * @param splitPacket
	 *            The message String split up by parentheses
	 ...
	 *
	 * @param splitName
	 *            this is the split up ObjName and side
	 * @param splitInfo
	 *            The rest of the information to store in the ObjLine
	 * @param newLine
	 *            The ObjLine to store the data in
	 ...
	 *
	 * @param splitName
	 *            this is the split up ObjName with flagName, flagType, side,
	 *            x_pos, y_pos, and yard
	 * @param splitInfo
	 *            The rest of the information to store in the ObjLine
	 * @param newFlag
	 *            The ObjFlag to store the data in
	 ...
	 *            this is the split up ObjName and side
	 * @param splitInfo
	 *            The rest of the information to store in the ObjBall
	 * @param newBall
	 *            The ObjBall to store the data in
	 ...
	 *
	 * @param splitName
	 *            this is the split up ObjName, team name, uniform number (if
	 *            visible), and goalie boolean (if visible).
	 * @param splitInfo
	 *            The rest of the information to store in the ObjPlayer
	 * @param newPlayer
	 *            The ObjPlayer to store the data in
	 ...
	 *            this is the split up ObjName and side
	 * @param splitInfo
	 *            The rest of the information to store in the ObjGoal
	 * @param newGoal
	 *            The ObjGoal to store the data in
	 ...
	 *
	 * @param newSenMem
	 *            this is the SenseMemory to store all the information in
	 * @param splitPacket
	 *            The message String split up by parentheses
